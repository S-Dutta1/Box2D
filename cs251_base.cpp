...
{
	b2Vec2 gravity;
	gravity.Set(0.0f, -8.0f);   // gravity has been changed for better visual
...
// keyboard functions implemented
void base_sim_t::keyboard(unsigned char key)
  ...
  //mouse functions implemented
  void base_sim_t::mouse_down(const b2Vec2& p){
    
    b2Vec2 center1(14.0f, 18.0f);
    b2Vec2 center2(-14.0f, 18.0f);

    float32 dist1,dist2;
    dist1 = b2Distance(center1,p);
    dist2 = b2Distance(center2,p);
...
void base_sim_t::step(settings_t* settings)
{
  ...
  m_world->Step(time_step, settings->velocity_iterations, settings->position_iterations);
      ...
      b2Body* magnetL=bodylist[n-2]; 
      b2Body* magnetR=bodylist[n-3];

      float32 force1,force2;
      b2Vec2 rvector1,rvector2;
      float32 k=10000.0f;
      for(int i=0;i<n-3;i++)
      {
           
           force1 = sign1*(k/ pow(b2Distance(magnetL->GetWorldCenter(),bodylist[i]->GetWorldCenter()),3));
           force2 = sign2*(k/ pow(b2Distance(magnetR->GetWorldCenter(),bodylist[i]->GetWorldCenter()),3));
           
           rvector1=magnetL->GetWorldCenter()-bodylist[i]->GetWorldCenter();
           rvector2=magnetR->GetWorldCenter()-bodylist[i]->GetWorldCenter();
           
           bodylist[i]->ApplyForce(force1*rvector1 + force2*rvector2 ,bodylist[i]->GetWorldCenter(),true);
      }
